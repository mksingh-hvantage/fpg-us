import { useEffect, useState, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { Plus } from 'lucide-react';
import DataTable from '../../../components/admin/DataTable';
import StatusBadge from '../../../components/admin/StatusBadge';
import { getCmsPages } from '../../../services/cmsService';
import { useAuth } from '../../../contexts/AuthContext';
import type { CmsPage } from '../../../types/admin';

export default function CmsPageList() {
  const [pages, setPages] = useState<CmsPage[]>([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [total, setTotal] = useState(0);
  const [search, setSearch] = useState('');
  const navigate = useNavigate();
  const { canCreate } = useAuth();

  const fetchPages = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, string | number> = { page, limit: 20 };
      if (search) params.search = search;
      const data = await getCmsPages(params);
      setPages(data.pages);
      setTotalPages(data.totalPages);
      setTotal(data.total);
    } catch (err) {
      console.error('Failed to fetch CMS pages:', err);
    } finally {
      setLoading(false);
    }
  }, [search, page]);

  useEffect(() => { fetchPages(); }, [fetchPages]);

  const columns = [
    { key: 'slug', label: 'Slug', sortable: true },
    { key: 'title', label: 'Title', sortable: true },
    { key: 'published', label: 'Published', render: (p: CmsPage) => <StatusBadge status={p.published ? 'PUBLISHED' : 'DRAFT'} /> },
    { key: 'updatedBy', label: 'Updated By', render: (p: CmsPage) => p.updatedBy?.fullName || '-' },
    { key: 'updatedAt', label: 'Updated', sortable: true, render: (p: CmsPage) => new Date(p.updatedAt).toLocaleDateString() },
  ];

  if (loading && pages.length === 0) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-600" />
      </div>
    );
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">CMS Pages</h2>
        {canCreate('CMS') && (
          <button
            onClick={() => navigate('/admin/cms/new')}
            className="flex items-center gap-2 bg-cyan-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-cyan-700"
          >
            <Plus className="w-4 h-4" />
            New Page
          </button>
        )}
      </div>

      <DataTable
        columns={columns}
        data={pages}
        searchPlaceholder="Search pages..."
        onSearch={(q) => { setSearch(q); setPage(1); }}
        page={page}
        totalPages={totalPages}
        total={total}
        onPageChange={setPage}
        onRowClick={(p) => navigate(`/admin/cms/${p.id}`)}
      />
    </div>
  );
}
